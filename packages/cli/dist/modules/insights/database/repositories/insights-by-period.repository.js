...
let InsightsByPeriodRepository = class InsightsByPeriodRepository extends typeorm_1.Repository {
    ...
    async getPreviousAndCurrentPeriodTypeAggregates({ periodLengthInDays, projectId, }) {
    ...
    async getInsightsByWorkflow({ maxAgeInDays, skip = 0, take = 20, sortBy = 'total:desc', projectId, }) {
    ...
    async getInsightsByTime({ maxAgeInDays, periodUnit, insightTypes, projectId, }) {
        const cte = (0, db_1.sql) `SELECT ${this.getAgeLimitQuery(maxAgeInDays)} AS start_date`;
